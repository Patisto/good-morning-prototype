...
type NewRoom = { id: string; housingUnit: string; identifier: string; label?: string; type: 'SINGLE' | 'DOUBLE'; capacity: number }

function buildRooms(): NewRoom[] {
  const rooms: NewRoom[] = []
  const numbered = (unit: string, prefix: string, count: number) => {
...
async function seed() {
  ...
  for (const r of rooms) {
    await pool.query(
      'INSERT INTO rooms (id, housing_unit, identifier, label, type, capacity) VALUES ($1, $2, $3, $4, $5, $6)',
      [r.id, r.housingUnit, r.identifier, r.label ?? null, r.type, r.capacity]
    )
  }
  ...
  for (const t of tenants) {
    const { rows } = await pool.query(
      `INSERT INTO tenants (first_name, last_name, phone, institution, programme, room_id, monthly_amount, move_in_date, agreement_status)
       VALUES ($1, $2, $3, $4, $5, $6, $7, $8, $9) RETURNING id`,
      [t.firstName, t.lastName, t.phone, t.institution, t.programme, t.roomId, t.monthlyAmount, t.moveInDate, t.agreementStatus]
    )
  ...
  for (const p of payments) {
    ...
    await pool.query(
      `INSERT INTO hostel_payments (tenant_id, period_label, expected_amount, amount_paid, payment_method, payment_date)
       VALUES ($1, $2, $3, $4, $5, $6)`,
      [tenantIds[p.name], 'September 2026', tenant.monthlyAmount, p.paid, p.paid > 0 ? 'CASH' : null, p.paid > 0 ? '2026-09-05' : null]
    )
  }

  console.log('Seeding food items and daily food records...')
  const foodItems: { name: string; unit: 'kg' | 'g' | 'litre' | 'piece'; cost: number }[] = [
  ...
  const foodItemIds: Record<string, string> = {}
  for (const f of foodItems) {
    const { rows } = await pool.query(
      'INSERT INTO food_items (name, unit, cost) VALUES ($1, $2, $3) RETURNING id',
      [f.name, f.unit, f.cost]
    )
  ...
  for (const r of foodRecords) {
    await pool.query(
      'INSERT INTO daily_food_records (date, food_item_id, quantity, cost) VALUES (CURRENT_DATE, $1, $2, $3)',
      [foodItemIds[r.name], r.quantity, r.cost]
    )
  }
  ...
  for (const m of menuItems) {
    const { rows } = await pool.query(
      'INSERT INTO menu_items (name, category, selling_price, available) VALUES ($1, $2, $3, true) RETURNING id',
  ...
  for (const p of products) {
    await pool.query(
      `INSERT INTO grocery_products (name, category, selling_price, buying_price, current_stock, low_stock_level, unit)
       VALUES ($1, $2, $3, $4, $5, $6, $7)`,
      [p.name, p.category, p.sellingPrice, p.buyingPrice, p.stock, p.lowStock, p.unit]
    )
  }
  ...
  for (const e of expenses) {
    await pool.query(
      'INSERT INTO expenses (description, amount, business, date, payment_method) VALUES ($1, $2, $3, $4, $5)',
      [e.description, e.amount, e.business, e.date, e.method]
    )
  }

  console.log('Seeding one sample sale...')
  const { rows: saleRows } = await pool.query(
    `INSERT INTO sales (business, date, time, total, payment_method, status, cashier)
     VALUES ('RESTAURANT', CURRENT_DATE, '12:30', 6100, 'CASH', 'COMPLETED', 'Grace M.') RETURNING id`
  )
  const saleId = saleRows[0].id
  await pool.query(
    `INSERT INTO sale_items (sale_id, item_id, name, quantity, unit_price) VALUES
       ($1, $2, 'Nsima + Beef', 2, 2500),
       ($1, $3, 'Banana', 3, 500)`,
    [saleId, menuItemIds['Nsima + Beef'], menuItemIds['Banana']]
  )
